...
async function loadClubs(day) {
    ...
    const clubs = await apiGet("getClubs", { day: day });
    box.innerHTML = '';
...
async function loadMembers(club) {
    ...
    box.innerHTML = '';
    selMembers = [];
    let members = await apiGet("getMembers", { club: club });
    if (members.length === 1 && typeof members[0] === 'string' && members[0].includes(',')) {
...
async function sendPost(mode) {
    ...
    if(mode === 'submitAttendance') {
        ...
        let phone = document.getElementById('b_phone').value.replace(/[^0-9]/g, '');
        if(phone.length === 11) {
        ...
        if(!data.name || !data.dateTime) return alert("필수 정보를 입력해주세요.");
        ...
        if(!data.club || !data.dateTime) return alert("필수 정보를 입력해주세요.");
    }

    // 봇 방지용 허니팟 값 추가 (서버 검증용)
    const honeypotVal = document.getElementById('honeypot')?.value;
    data.honeypot = honeypotVal;
    ...
    submitBtns.forEach(btn => {
        btn.disabled = true;
        originalTexts.push(btn.innerText);
        btn.innerText = "처리 중...";
    });
    ...
    try {
        ...
        console.error(e);
        alert("죄송합니다. 잠시 후에 다시 눌러주시면 바로 완료됩니다.");
    } finally {
...
function submitAttendance() {
    if(!selMembers.length) return alert("인원을 선택하세요.");
